...
Public Class FDichiarazione
    ...
    Public Sub Apri(Dic As DichiarazioneEntity)
        ...
        'PP.Add(DichiarazioneEntity.PrefetchPathAllegato).SubPath.Add(AllegatoEntity.PrefetchPathDettaglio)
    ...
    Private Sub Inizializza()
        InitUiSecurity()
        Me.Text = String.Format("Dichiarazione periodica - {0}", _Dichiarazione.Id)
        ...
        _Dichiarazione.SaveFields("edit")
        ...
        tsbAllegato.Available = DichiarazioneEntity.CanHaveAllegato(_Dichiarazione.CodCategoria)
        tsbAllegato.BackColor = IIf(_Dichiarazione.HasAllegato, Color.FromArgb(255, 255, 192), Color.FromKnownColor(KnownColor.Control))

        tsbProdotti.Available = DichiarazioneEntity.CanHaveProdotti(_Dichiarazione.CodCategoria)
        tsbProdotti.BackColor = IIf(_Dichiarazione.HasProdottiDichiarati, Color.FromArgb(255, 255, 192), Color.FromKnownColor(KnownColor.Control))
    ...
    Private Sub UpdateLayout()
        ...
        btnStatoDoc.Image = ImagesHelper.GetStatoDicPagIcon(_Dichiarazione.CodStatoDichiarazione)
        lStatoIncasso.Image = ImagesHelper.GetStatoDicPagIcon(_Dichiarazione.CodStatoPagamento)
    ...
    Private Sub tsbExit_Click(sender As System.Object, e As System.EventArgs) Handles tsbExit.Click
        If _Dichiarazione.IsNew Then
            _Dichiarazione = Nothing
        Else
            _Dichiarazione.RollbackFields("edit")
        End If
    ...
    Private Sub btnStatoDoc_Click(sender As Object, e As EventArgs) Handles btnStatoDoc.Click
        ...
        ElseIf _Dichiarazione.CodStatoDichiarazione = Enums.StatoDocumento.PARTIAL Then
            _Dichiarazione.CodStatoDichiarazione = Enums.StatoDocumento.FORCED
...
#End Region
    ...
    Private Sub rifArxivar_ProfileClick(sender As Object, e As ArxivarWCFWrapper.ucProtocolloArxivar.ProfileClickEventArgs) Handles rifArxivar.ProfileClick
        Try
            ...
            Dim PProf As New Dictionary(Of String, Object)
            PProf.Add("DataDoc", dt.Value)
            PProf.Add("DocName", "DICHIARAZIONE PERIODICA " & _Dichiarazione.GetCompetenzaDescription)
            PProf.Add("CodMittente", CommonConfig.ArxivarIdRubricaSoci & "," & CommonConfig.PrefissoCodiceRubrica & _Dichiarazione.IdAzienda)
    ...
    Private Sub txtDataRicezione_LostFocus(sender As Object, e As System.EventArgs) Handles txtDataRicezione.LostFocus
        UpdateLayout()
    End Sub

    Private Sub tsbAllegato_Click(sender As Object, e As EventArgs) Handles tsbAllegato.Click
        Try
            Dim F As New dlgAllegato
            If _Dichiarazione.HasAllegato Then
                F.Apri(_Dichiarazione.Allegato)
            Else
                If Not _Dichiarazione.DataRicezione.HasValue Then _Dichiarazione.DataRicezione = Date.Today
                F.Nuovo(_Dichiarazione)
            End If
            tsbAllegato.BackColor = IIf(_Dichiarazione.HasAllegato, Color.FromArgb(255, 255, 192), Color.FromKnownColor(KnownColor.Control))
        Catch ex As Exception
    ...
    Private Sub TsbProdotti_Click(sender As Object, e As EventArgs) Handles tsbProdotti.Click
        Try
            Dim F As New dlgProdottiDichiarati
            F.Apri(_Dichiarazione)
            tsbProdotti.BackColor = IIf(_Dichiarazione.HasProdottiDichiarati, Color.FromArgb(255, 255, 192), Color.FromKnownColor(KnownColor.Control))
        Catch ex As Exception
            MsgBox(ex.Message, MsgBoxStyle.Exclamation)
        End Try
