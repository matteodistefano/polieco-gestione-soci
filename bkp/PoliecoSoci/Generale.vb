...
Module Generale
    ...
    Public Sub MessageBox(ByVal FOwner As IWin32Window, ByVal Msg As String, ByVal Icon As MessageBoxIcon)
        Dim Risp
        Risp = System.Windows.Forms.MessageBox.Show(FOwner, Msg, "EBNT", MessageBoxButtons.OK, Icon)
    End Sub
    ...
    Public Sub InizializzaApplicazione()
        ...
        mdsUIControls.ucTxtCall.DefaultAction(mdsUIControls.ucTxtCall.TipoEnum.Telefono) = Sub(sender As System.Object, e As mdsUIControls.MdsEventArgs.HandledEventArgs)
                                                                                               Try
                                                                                                   e.IsHandled = True
                                                                                                   Dim nn() As String = AtapiManager.GetCallableNumber(sender.text)
                                                                                                   If nn Is Nothing OrElse nn.Length = 0 Then Return
                                                                                                   Dim lineId As Integer = Generale.UserConfig.IdLineaTAPI
                                                                                                   If nn.Length = 1 Then
                                                                                                       AtapiManager.MakeCall(nn(0), lineId)
                                                                                                   Else
                                                                                                       Dim cm As ContextMenuStrip = AtapiManager.GetContextMenu(nn, lineId)
